<script lang="ts">
	...
	const hasAccess = $derived(() => {
		...
		if (tier === 'professional' || tier === 'trial') return requiredPlan === 'professional';
		return false;
	});
</script>
	...
	>
		...
		<div>
			...
			<p class="mt-1 text-xs text-muted-foreground">
				{#if status?.tier === 'none'}
					Aucun abonnement actif. Démarrez un essai gratuit de 15 jours ou choisissez un plan.
				{:else}
		...
		<div class="flex gap-2">
			{#if status?.tier === 'none'}
				...
				<a href={localizedHref('/admin/settings/plans?trial=1')}>
					<Button size="sm" variant="outline">Essai gratuit 15 jours</Button>
				</a>
			{/if}
