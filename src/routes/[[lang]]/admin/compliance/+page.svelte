<script lang="ts">
	...
	const licensesQ = useQuery(anyApi.agents.compliance.toolGetDriverLicenseStatus, { daysAhead: 90 });
...
<div class="space-y-6 px-4 py-6 md:px-6">
	...
	<div class="flex items-start justify-between gap-4">
		<div>
			...
			<p class="mt-0.5 text-sm text-muted-foreground">Documents, permis, contraventions, maintenance.</p>
		</div>
	...
	{:else if dashQ.data}
		{@const d = dashQ.data}
		<div class={['flex items-center gap-4 rounded-xl border p-4', RISK_BG[d.riskLevel] ?? ''].join(' ')}>
			...
			<div class="flex-1">
				...
				<p class="text-xs text-muted-foreground">
					Score {d.riskScore} — {d.summary.vehiclesChecked} véhicules · {d.summary.driversChecked} conducteurs · {d.currentDate}
				</p>
			</div>
			{#if d.riskLevel !== 'COMPLIANT'}
				<Button variant="outline" size="sm" onclick={() => copilot.open('compliance')} class="shrink-0">
					Analyser avec IA
				</Button>
			{/if}
		</div>
	...
	<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
		...
		{:else}
			<div class="rounded-xl border border-border/60 bg-card p-4">
				...
				<p class={['mt-2 text-2xl font-semibold tabular-nums', (docsQ.data?.summary.expiredCount ?? 0) > 0 ? 'text-destructive' : ''].join(' ')}>
					{docsQ.data?.summary.expiredCount ?? '—'}
				</p>
				<p class="mt-0.5 text-xs text-muted-foreground">
					expirés · <span class="text-orange-500">{docsQ.data?.summary.expiringCount ?? '—'}</span> dans 60j
				</p>
			</div>
			<div class="rounded-xl border border-border/60 bg-card p-4">
				<div class="flex items-center gap-2 text-xs text-muted-foreground">
				...
				<p class={['mt-2 text-2xl font-semibold tabular-nums', (licensesQ.data?.summary.expiredLicenses ?? 0) > 0 ? 'text-destructive' : ''].join(' ')}>
					{licensesQ.data?.summary.expiredLicenses ?? '—'}
				</p>
				<p class="mt-0.5 text-xs text-muted-foreground">
					expirés · <span class="text-orange-500">{licensesQ.data?.summary.expiringLicenses ?? '—'}</span> dans 90j
					{#if (licensesQ.data?.summary.blockedDrivers ?? 0) > 0}
						· <span class="text-destructive">{licensesQ.data?.summary.blockedDrivers} bloqués</span>
					{/if}
				</p>
			</div>
			<div class="rounded-xl border border-border/60 bg-card p-4">
				...
				<p class={['mt-2 text-2xl font-semibold tabular-nums', (violationsQ.data?.summary.total ?? 0) > 0 ? 'text-orange-500' : ''].join(' ')}>
					{violationsQ.data?.summary.total ?? '—'}
				</p>
				<p class="mt-0.5 text-xs text-muted-foreground">
					en attente · {(violationsQ.data?.summary.totalFinesAmount ?? 0).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}
				</p>
			</div>
			<div class="rounded-xl border border-border/60 bg-card p-4">
				<div class="flex items-center gap-2 text-xs text-muted-foreground">
				...
				<p class={['mt-2 text-2xl font-semibold tabular-nums', (maintenanceQ.data?.summary.overdueRecords ?? 0) > 0 ? 'text-orange-500' : ''].join(' ')}>
	...
	{#if docsQ.data?.expired && docsQ.data.expired.length > 0}
		<div class="space-y-2">
			...
			<div class="divide-y divide-border/50 overflow-hidden rounded-xl border border-destructive/20 bg-destructive/5">
				{#each docsQ.data.expired.slice(0, 8) as item (item.vehicle + item.document)}
					<div class="flex items-center justify-between px-4 py-2.5">
						...
						<Badge variant="destructive" class="text-[10px]">{item.expiredDaysAgo}j de retard</Badge>
					</div>
				{/each}
			</div>
		</div>
	...
	{#if docsQ.data?.expiring && docsQ.data.expiring.length > 0}
		<div class="space-y-2">
			...
			<div class="divide-y divide-border/50 overflow-hidden rounded-xl border border-orange-500/20 bg-orange-500/5">
				{#each docsQ.data.expiring as item (item.vehicle + item.document)}
					<div class="flex items-center justify-between px-4 py-2.5">
						...
						<Badge variant="outline" class={['text-[10px]', item.daysLeft <= 14 ? 'border-orange-500/40 text-orange-500' : ''].join(' ')}>
							{item.daysLeft}j
						</Badge>
					</div>
				{/each}
	...
	{#if licensesQ.data && (licensesQ.data.expired.length > 0 || licensesQ.data.blocked.length > 0 || licensesQ.data.expiring.length > 0)}
		<div class="space-y-2">
			...
			<div class="divide-y divide-border/50 overflow-hidden rounded-xl border border-border/60 bg-card">
				...
				{#each licensesQ.data.expired as d (d.driver + 'e')}
					<div class="flex items-center justify-between px-4 py-2.5">
						<p class="text-sm font-medium">{d.driver}</p>
						<Badge variant="destructive" class="text-[10px]">Permis expiré il y a {d.expiredDaysAgo}j</Badge>
					</div>
				{/each}
	...
	{#if dashQ.data?.riskLevel === 'COMPLIANT'}
		<div class="flex flex-col items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 py-12 text-center">
			<ShieldCheckIcon class="size-12 text-emerald-500" />
			<p class="text-base font-semibold text-emerald-600 dark:text-emerald-400">Flotte conforme</p>
			<p class="max-w-sm text-sm text-muted-foreground">Aucun document expiré, permis valides, pas de contravention en attente.</p>
		</div>
	{/if}
	...
	<div class="rounded-lg border border-[var(--brand)]/20 bg-[var(--brand)]/5 p-4">
		<div class="flex items-start gap-3">
			...
			<div class="flex-1">
				...
				<p class="mt-0.5 text-xs text-muted-foreground">
					Questions en langage naturel : « Quels véhicules ont un CT expiré ? », « Résume les risques de conformité », « Calcule le BiK UK de notre flotte ».
				</p>
			</div>
			<Button variant="outline" size="sm" onclick={() => copilot.open('compliance')} class="shrink-0">Ouvrir</Button>
		</div>
	</div>
</div>
