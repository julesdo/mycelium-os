...
<SubscriptionGate feature="bik" requiredPlan="professional">
	<div class="flex flex-col gap-6 p-6">
		...
		{#if !loading && (report?.summary.missingP11dCount ?? 0) > 0}
			<div
				class="flex items-center gap-3 rounded-xl border border-amber-500/20 bg-amber-500/8 px-4 py-3"
			>
				<AlertTriangleIcon class="size-4 shrink-0 text-amber-500" />
				<p class="text-sm text-amber-700 dark:text-amber-400">
					<strong
						>{report!.summary.missingP11dCount} vehicle{report!.summary.missingP11dCount > 1
							? 's'
							: ''}</strong
					>
					{report!.summary.missingP11dCount > 1 ? 'are' : 'is'} missing P11D value or CO2 data — click
					<strong>Edit</strong> in the table to add them.
				</p>
			</div>
		{/if}
		...
		<div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
			...
			{:else}
				...
				<MetricCard
					...
					description="{report?.summary.electricCount ?? 0} of {report?.summary.totalVehicles ??
						0} vehicles"
				/>
			{/if}
		</div>
		...
		{#if !loading && (report?.evOpportunities.length ?? 0) > 0}
			<div class="rounded-2xl border bg-card p-4">
				...
				<div class="mb-3 flex items-center gap-2">
					...
					<span class="ml-auto text-xs text-muted-foreground">Potential NIC savings per year</span>
				</div>
				<div class="flex flex-col gap-2">
					{#each report!.evOpportunities as opp, i (i)}
						<div class="flex items-center justify-between rounded-xl bg-muted/40 px-3 py-2.5">
							<div>
								...
								<span class="ml-2 text-xs text-muted-foreground">{opp.model}</span>
							</div>
							<div class="flex items-center gap-3">
								<span class="text-xs text-muted-foreground"
									>{opp.currentBikRate}% → {opp.evBikRate}%</span
								>
					...
					{#if (report?.summary.potentialNicSaving ?? 0) > 0}
						<p class="mt-1 text-xs text-muted-foreground">
							...
							<strong class="text-emerald-600 dark:text-emerald-400"
								>{fmt(report!.summary.potentialNicSaving)}/yr</strong
							>
						</p>
					{/if}
				</div>
		...
		<div class="overflow-hidden rounded-2xl border bg-card">
			...
			<div class="flex items-center justify-between border-b px-4 py-3">
				<span class="text-sm font-medium"
					>Fleet BiK Breakdown — {TAX_YEARS.find((y) => y.value === taxYear)?.label}</span
				>
				<span class="text-xs text-muted-foreground">
					{report?.summary.vehiclesWithP11d ?? 0} / {report?.summary.totalVehicles ?? 0} vehicles with
					P11D data
				</span>
			</div>
				...
				<table class="w-full text-sm">
					<thead>
						<tr class="border-b">
							...
							>
							<th
								class="px-3 py-2.5 text-left text-xs font-medium tracking-wide text-muted-foreground uppercase"
								>Fuel</th
							>
							<th
								class="px-3 py-2.5 text-right text-xs font-medium tracking-wide text-muted-foreground uppercase"
								>CO2</th
							>
							<th
								class="px-3 py-2.5 text-right text-xs font-medium tracking-wide text-muted-foreground uppercase"
					...
					<tbody>
						...
						{:else}
							{#each report.vehicles as row (row._id ?? row.registration)}
								<tr class="border-b transition-colors last:border-0 hover:bg-muted/30">
									<td class="px-4 py-3">
										<div class="font-medium">{row.registration}</div>
										<div class="text-xs text-muted-foreground">{row.brand} {row.model}</div>
									</td>
									<td class="px-3 py-3">
									...
									<td class="px-3 py-3 text-right">
										...
										{:else}
											<span class="text-xs font-medium text-amber-500">Missing</span>
										{/if}
									</td>
									<td class="px-3 py-3 text-right">
										{#if row.bikRate != null}
									...
									<td class="px-3 py-3 text-right font-mono text-xs">
										{#if row.employerNic != null}
											<span class={row.employerNic > 500 ? 'font-semibold text-destructive' : ''}>
												{fmt(row.employerNic)}
											</span>
										{:else}
						...
						<tfoot>
							<tr class="border-t bg-muted/20">
								...
								<td class="px-3 py-2.5 text-right font-mono text-xs font-semibold"
									>{fmt(report.summary.totalBikValue)}</td
								>
								<td class="px-3 py-2.5 text-right font-mono text-xs font-semibold"
									>{fmt(report.summary.totalEmployeeBasicTax)}</td
								>
								<td class="px-3 py-2.5 text-right font-mono text-xs font-semibold text-destructive"
									>{fmt(report.summary.totalEmployerNic)}</td
								>
								<td></td>
							</tr>
						</tfoot>
		...
		<div class="flex items-start gap-2 rounded-xl border border-dashed px-4 py-3">
			...
			<p class="text-xs leading-relaxed text-muted-foreground">
				<strong class="text-foreground">How BiK works:</strong> The P11D value (HMRC list price) ×
				BiK % = taxable benefit. Employees pay income tax on this amount; employers pay Class 1A NIC
				(13.8%) by 19 July each year. Electric vehicles attract only {taxYear === '2024-25'
					? '2%'
					: taxYear === '2025-26'
						? '3%'
						: taxYear === '2026-27'
							? '4%'
							: '5%'}
				BiK in {TAX_YEARS.find((y) => y.value === taxYear)?.label}. Diesel cars incur a +4%
				surcharge unless RDE2-compliant.
			</p>
		</div>
	</div>
		...
		<Dialog.Content class="max-w-sm">
			...
			<Dialog.Footer>
				...
				<Button onclick={saveEdit} disabled={saving || !editP11d}>
					{#if saving}Saving…{:else}<CheckIcon class="mr-1.5 size-4" />Save{/if}
				</Button>
			</Dialog.Footer>
		</Dialog.Content>
	</Dialog.Root>
