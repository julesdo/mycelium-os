<script lang="ts">
	...
	const sortedVehicles = $derived.by(() => {
		...
		return [...data.perVehicle].sort((a, b) => {
			...
			if (sortKey === 'tco2e') { av = a.tco2e; bv = b.tco2e; }
			else if (sortKey === 'consumption') {
				av = a.litersConsumed ?? a.kwh ?? 0;
				bv = b.litersConsumed ?? b.kwh ?? 0;
			} else if (sortKey === 'registration') { av = a.registration; bv = b.registration; }
			else if (sortKey === 'energy') { av = a.energy; bv = b.energy; }
			else { av = a.scope; bv = b.scope; }
	...
	async function handleGeneratePDF() {
		...
		generating = true;
		try {
			const { generateCarbonPDF } = await import(
				'$lib/components/sustainability/carbon-pdf-generator'
			);
...
<SEOHead title="Bilan Carbone — Mycelium Fleet" description="Rapport d'émissions CO2 Scope 1 & 2 de la flotte" />
	...
	{:else if data}
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
			...
			<MetricCard
				...
				description="{data.perVehicle.filter((v: { scope: string }) => v.scope === 'SCOPE1').length} véhicule(s) thermique(s)"
			/>
			<MetricCard
				label="Scope 2 — Électricité VE"
				value="{data.scope2TotalTCO2e} tCO₂e"
				description="{data.perVehicle.filter((v: { scope: string }) => v.scope === 'SCOPE2').length} véhicule(s) électrique(s)"
			/>
		</div>
		...
		{#if csrdData && csrdData.emissions.scope3.total > 0}
			<div class="rounded-2xl border bg-card p-4">
				...
				<div class="mb-3 flex items-center justify-between">
					...
					<a href={resolve(`/${lang}/admin/sustainability/esrs-e1?year=${selectedYear}`)} class="text-muted-foreground hover:text-foreground text-xs underline-offset-2 hover:underline">
						Rapport ESRS E1 complet →
					</a>
				</div>
				<div class="flex flex-col gap-1.5">
					{#each csrdData.emissions.scope3.breakdown as entry}
						<div class="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-1.5 text-xs">
		...
		{#if data.dataSource === 'COST_ESTIMATE'}
			<div class="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/8 px-4 py-3">
				...
				<div class="text-xs">
					...
					<span class="ml-1 text-muted-foreground">— volumes calculés depuis les coûts carburant (prix moyen ADEME). Importez vos relevés carte carburant pour une précision optimale.</span>
				</div>
			</div>
		{:else}
			<div class="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/8 px-4 py-3">
				...
				<p class="text-xs">
					...
					<span class="ml-1 text-muted-foreground">— volumes issus de vos imports de relevés carte carburant. Facteurs ADEME Base Carbone {data.ademeYear}.</span>
				</p>
			</div>
		{/if}
		...
		{#if data.totalTCO2e === 0}
			<div class="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-border py-16 text-center">
				<LeafIcon class="size-10 text-muted-foreground/40" />
				<p class="text-sm font-medium">Aucune donnée carburant pour {selectedYear}</p>
				<p class="max-w-xs text-xs text-muted-foreground">Renseignez des coûts carburant ou importez vos relevés carte carburant pour calculer les émissions.</p>
			</div>
		{:else}
			<!-- ADEME info -->
			<div class="flex items-center gap-2 text-xs text-muted-foreground">
				<InfoIcon class="size-3.5 shrink-0" />
				<span>Facteurs d'émission ADEME Base Carbone {data.ademeYear} — Diesel : 2,640 kg CO₂e/L · Essence : 2,289 kg CO₂e/L · Électricité (mix FR) : 0,051 kg CO₂e/kWh</span>
			</div>
					...
					<table class="w-full text-xs">
						<thead>
							<tr class="border-b border-border bg-muted/30">
								{#each [
									{ key: 'registration', label: 'Véhicule' },
									{ key: 'energy', label: 'Énergie' },
									{ key: 'scope', label: 'Scope' },
									{ key: 'consumption', label: 'Consommation' },
									{ key: 'tco2e', label: 'tCO₂e' }
								] as col (col.key)}
									<th
										class="cursor-pointer select-none px-4 py-2.5 text-left font-semibold uppercase tracking-wider text-muted-foreground hover:text-foreground"
						...
						<tbody>
							{#each sortedVehicles as v, i (v.vehicleId)}
								<tr class="border-b border-border/60 transition-colors hover:bg-muted/30 {i % 2 === 0 ? '' : 'bg-muted/10'}">
									<td class="px-4 py-3">
										<span class="font-medium">{v.brand} {v.model}</span>
										<span class="ml-2 font-mono text-[10px] text-muted-foreground">{v.registration}</span>
									</td>
									<td class="px-4 py-3 text-muted-foreground">{ENERGY_LABELS[v.energy] ?? v.energy}</td>
									<td class="px-4 py-3">
										<span class="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold {v.scope === 'SCOPE2' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'bg-orange-500/10 text-orange-600 dark:text-orange-400'}">
						...
						<tfoot>
							<tr class="border-t-2 border-border bg-muted/20">
								<td colspan="4" class="px-4 py-3 text-xs font-semibold">Total flotte {selectedYear}</td>
								<td class="px-4 py-3 text-right font-mono text-sm font-bold">{data.totalTCO2e.toFixed(2)}</td>
							</tr>
						</tfoot>
					</table>
				</div>
		...
		{#if reports.length > 0}
			<div class="rounded-2xl border border-border bg-card px-4 py-4">
				<h2 class="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">Rapports générés</h2>
				<div class="flex flex-wrap gap-2">
					{#each reports as r (r._id)}
						<div class="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-1.5 text-xs">
							...
							<span class="text-muted-foreground">{new Date(r.generatedAt).toLocaleDateString('fr-FR')}</span>
						</div>
					{/each}
				</div>
			</div>
