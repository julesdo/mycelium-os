<script lang="ts">
	...
	// Fleet Care — fired unconditionally, data === undefined if no access (requireOrgAdmin throws)
	...
	const hasFleetCare = $derived(!summaryQ.isLoading && summary !== undefined);

	const viewer = $derived(page.data?.viewer as { name?: string } | null);
	const firstName = $derived(viewer?.name?.split(' ')[0] ?? '');

	// ── Tab state ────────────────────────────────────────────────────────────────
	let activeTab = $state<'home' | 'fleet-care'>('home');
	...
	// ── Fleet Care score color ───────────────────────────────────────────────────
	const scoreColor = $derived(
		!summary ? '' :
		summary.healthScore >= 90 ? 'text-emerald-500' :
		summary.healthScore >= 75 ? 'text-sky-500' : 'text-amber-500'
	);
</script>
...
<div class="flex flex-col gap-5 px-4 pb-24 pt-5 lg:pb-10 lg:px-6 xl:px-8 2xl:px-16">
	...
	<div class="flex items-start justify-between gap-4">
		<div>
			...
			<p class="mt-0.5 text-sm capitalize text-muted-foreground">
				{activeTab === 'fleet-care' && summary ? summary.orgName : dateLabel}
			</p>
		</div>
		...
		{:else if summary}
			<!-- Health score chip -->
			<div class="relative mt-1 overflow-hidden rounded-xl border border-border bg-card px-3 py-1.5 shadow-glass-card">
				...
				<span class="text-sm font-black tabular-nums {scoreColor}">{summary.healthScore}</span>
				<span class="text-xs font-medium text-muted-foreground">/100</span>
			</div>
		{/if}
	</div>

	<!-- ── Segment switcher (admin only, reveals once fleet-care data loads) ─── -->
	{#if hasFleetCare}
		<div
			class="relative flex w-full rounded-2xl p-1 ring-1 ring-border/60"
			style="background: color-mix(in oklch, var(--muted) 60%, transparent); box-shadow: inset 0 1px 3px oklch(0 0 0 / 0.10)"
		>
			<!-- Sliding pill -->
			<div
				class="absolute inset-y-1 w-[calc(50%-4px)] rounded-xl transition-all duration-200 ease-out"
				style="left: {activeTab === 'home' ? '4px' : 'calc(50% + 3px)'}; background: var(--card); box-shadow: 0 1px 4px oklch(0 0 0 / 0.12), 0 0 0 1px oklch(0 0 0 / 0.05), inset 0 1px 0 oklch(1 0 0 / 0.85)"
			>
				<div class="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/90 to-transparent dark:via-white/18"></div>
			</div>

			<button
				type="button"
				onclick={() => (activeTab = 'home')}
				class="relative z-10 flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition-colors duration-150 {activeTab === 'home' ? 'text-foreground' : 'text-muted-foreground'}"
			>
			...
			<button
				type="button"
				onclick={() => (activeTab = 'fleet-care')}
				class="relative z-10 flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition-colors duration-150"
				style={activeTab === 'fleet-care' ? 'color: var(--brand-foreground)' : ''}
				class:text-muted-foreground={activeTab !== 'fleet-care'}
			>
				<HeartHandshakeIcon class="size-4 shrink-0" />
				Fleet Care
			</button>
		</div>
	{/if}
			...
			<div class="flex flex-col gap-4">
				...
				>
					...
					<div class="relative flex items-center gap-3">
						...
						<ArrowRightIcon class="size-4 shrink-0 text-[var(--brand-foreground)]/50 dark:text-[var(--brand)]/50 transition-transform group-hover:translate-x-0.5" />
					</div>
				</button>
